...
<body>
 <script language="javascript" type="text/javascript">
     function SendMails() {
       ...
       // var pOpen = inside.match(/^<p>/)
       // var pClose = inside.match(/<\/p>$/)
         var rExp = /<br\s?\/>[\r\n]+\s*(<br\s?\/>[\r\n]+){1,2}/gi;
         var rRepl = "</p><p>"
         var content = "<p>" + $find("<%=Editor.ClientID%>").get_content().replace(rExp, rRepl) + "</p>"; document.forms[0].txtMsg.value = content;
     ...
     function Preview() {
         var rExp = /<br\s?\/>[\r\n]+\s*(<br\s?\/>[\r\n]+){1,2}/gi;
         var rRepl = "</p><p>"
         var content = "<p>" + $find("<%=Editor.ClientID%>").get_content().replace(rExp, rRepl) + "</p>";         //var content = document.forms[0].Editor.value;
     ...
     function SaveDraft() {
        ...
        var rExp = /<br\s?\/>[\r\n]+\s*(<br\s?\/>[\r\n]+){1,2}/gi;
         var rRepl = "</p><p>"
         var content = "<p>" + $find("<%=Editor.ClientID%>").get_content().replace(rExp, rRepl) + "</p>";
