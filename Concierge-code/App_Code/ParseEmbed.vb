...
Public Class ParseEmbed 'TODO lose the _BD
    ...
    Private Function GetThumb(ByVal n As Integer) As String
        mThumb = rImg.Match(sThumbURL) 'Check
        Dim iMediaTemp As Integer = iMedia
        Dim sTempReturn As String
        iMedia = 3 'THIS IS A TEMPORARY FIX for the case problem in MakeThumb - makes thumbnail when valid thumb entry or (no valid entry and youtube embed) 

        If mThumb.Success Then  'if valid image url entered in textbox
            sTempReturn = MakeThumb(mThumb.Value)
            iMedia = iMediaTemp
            Return sTempReturn
        ElseIf rEmbedYouTube.Match(sInput).Success And (3 > iMediaTemp AndAlso 1 <= iMediaTemp) Then  'if empty (or invalid) entry AND type LIKE "*Video" AND sInput is YouTube embed
            m = rImgYouTube.Match(sInput)
            sTempReturn = MakeThumb("http://img.youtube.com/vi/" + m.Value + "/2.jpg")
            iMedia = iMediaTemp
            Return sTempReturn
        Else  'otherwise use appropriate match given iMedia value
            iMedia = iMediaTemp
            m = regMObjs(n).Match(sInput)
