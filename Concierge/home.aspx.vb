...
Partial Class home
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            If (Request.Cookies("LoginAuth") Is Nothing) Then
                Response.Redirect("~/Login.aspx")
            End If
            rdbProfileType.SelectedIndex = 1
            Dim sSQL As String
            If Request.QueryString.Count > 0 Then
                If Not (Request.QueryString("sentid") Is Nothing) Then
                    ...
                    'Dim sBody As String = sData.Rows(0)("body").ToString
                    'Editor.EditPanel.Content = Regex.Replace(sBody, "(<p>)(?<inner>.*?)(</p>)", "${inner}<br /><br />")
                    ...
                    pnlDTitle.Visible = True
                    btnSend.Attributes.Add("onclick", "return SendMails()")
                    'btnSaveDraft.Visible = False
                    btnSaveDraft.Attributes.Add("onclick", "return SaveDraft(1)")
                ...
                ElseIf Not (Request.QueryString("draftid") Is Nothing) Then
                    ...
                    'Dim sBody As String = sData.Rows(0)("body").ToString
                    'Editor.EditPanel.Content = Regex.Replace(sBody, "(<p>)(?<inner>.*?)(</p>)", "${inner}<br /><br />")
                    txtDraftTitle.Text = sData.Rows(0)("DraftTitle").ToString
                    txtMedia.Text = sData.Rows(0)("media_text").ToString
                    ...
                    btnSend.Attributes.Add("onclick", "return SendMails()")
                    btnPreview.Attributes.Add("onclick", "return Preview()")
                    btnUpdateDraft.Attributes.Add("onclick", "return SaveDraft(1)")
                    btnSaveDraft.Text = "Save As"
                    btnSaveDraft.Attributes.Add("onclick", "return SaveDraft(0)")
            ...
            Else
                ...
                End If
                'If MyReferrer.Contains("catalog_jg.aspx") Then
                txtMedia.Text = Session("FLOATBOX")
                litPreview.Text = Session("FLOATBOX")
                'Response.Write(Session("R21CATEGORY"))
                Session("FLOATBOX") = ""

                sSQL = "SELECT * FROM (SELECT 111111 'TemplateID', 'Select a Template' 'TemplateName' UNION SELECT TemplateID, CAST(TemplateID AS VARCHAR(10)) + ' : ' + TemplateName + ' : ' + CAST(created_at AS VARCHAR(50)) TemplateName FROM ConciergeTemplate) DT ORDER BY TemplateID DESC"
                Dim sData As DataTable = GetData(sSQL)
                ddnTemplate.DataSource = GetData(sSQL)
                ddnTemplate.DataTextField = "TemplateName"
                ddnTemplate.DataValueField = "TemplateID"
                ddnTemplate.DataBind()
                'End If
            End If
        End If
    End Sub
    Private Sub PopulateWrapper(ByVal sSelected As String)
        ...
        For I = 0 To lstWrapper.Items.Count - 1
            ...
            End If
        Next
        'Rajiv 9/27/2011: Take care of custom wrapper text
        If txtWrapper.Text = "" Then
            txtWrapper.Text = sSelected
        End If
    End Sub
    ...
    Private Function GetSQL(ByVal Interests As String, ByVal UsersList As String) As String
        ...
        ElseIf UsersList = "B3,T10,pGPS" Then
            ...
            sSQL = sSQLMain
        ElseIf UsersList.Contains("All") = True Then
            sSQL = sSQLMain
        End If
        ...
        Try
            If ListCategory.SelectedIndex > -1 Then
                ...
                For Each lstItem In ListCategoryInterest.Items
                    ...
                    If Not ListInterests.Items.FindByValue(iIntID) Is Nothing Then
    ...
    Protected Sub btnRemoveCategory_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRemoveCategory.Click
        Try
            If ListCategoryNew.SelectedIndex > -1 Then
                ...
                For Each lstItem In ListCategoryInterest.Items
                    Dim iIntID As Integer = lstItem.Value
                    If Not ListInterestsNew.Items.FindByValue(iIntID) Is Nothing Then
    ...
    Protected Sub ListGroups_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListGroups.DataBound
        'If LCase(Request.ServerVariables("HTTP_REFERER")).Contains("catalog_jg.aspx") Then
        If Not Session("ASSOCIATION") Is Nothing Then
            Dim MyArray() As String = Split(Session("ASSOCIATION"), ",")
            Dim listitem As New ListItem
            Dim I As Integer = 0
            For I = 0 To UBound(MyArray)
                listitem = ListGroups.Items.FindByValue(Trim(MyArray(I)))
                If Not listitem Is Nothing Then
                    listitem.Selected = True
                    'ListGroups.SelectedValue = listitem.Value
                End If

                Call btnAddGroup_Click(sender, e)
            Next
        End If
        'End If
    ...
    Protected Sub ListCategory_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListCategory.DataBound
        'If LCase(Request.ServerVariables("HTTP_REFERER")).Contains("catalog_jg.aspx") Then
        If Not Session("R21CATEGORY") Is Nothing Then
            Dim MyArray() As String = Split(Session("R21CATEGORY"), ",")
            Dim listitem As New ListItem
            Dim I As Integer = 0
            For I = 0 To UBound(MyArray)
                listitem = ListCategory.Items.FindByValue(Trim(MyArray(I)))
                If Not listitem Is Nothing Then
                    listitem.Selected = True
                    'ListCategory.SelectedValue = listitem.Value
                End If

                Call btnAddCategory_Click(sender, e)
            Next
        End If
        'End If
    End Sub

    Protected Sub ListInterests_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListInterests.DataBound
        'If LCase(Request.ServerVariables("HTTP_REFERER")).Contains("catalog_jg.aspx") Then
        If Not Session("INTERESTS") Is Nothing Then
            Dim MyArray() As String = Split(Session("INTERESTS"), ",")
            Dim listitem As New ListItem
            Dim I As Integer = 0
            For I = 0 To UBound(MyArray)
                listitem = ListInterests.Items.FindByValue(Trim(MyArray(I)))
                If Not listitem Is Nothing Then
                    ListInterests.SelectedValue = listitem.Value
                End If

                Call btnAddInterest_Click(sender, e)
            Next
        End If
        'End If
    End Sub
    ...
    Protected Sub ddnTemplate_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddnTemplate.SelectedIndexChanged
        ...
        If sData.Rows.Count > 0 Then
            txtIconURL.Text = sData.Rows(0)("icon_url").ToString
            txtWrapper.Text = sData.Rows(0)("wrapper_text").ToString
            txtSubject.Text = sData.Rows(0)("title").ToString
            Editor.Content = sData.Rows(0)("body").ToString
            imgIcon.ImageUrl = sData.Rows(0)("icon_url").ToString
            
        Else
            txtIconURL.Text = "http://r21concierge.s3.amazonaws.com/images/concierge.png"
            txtWrapper.Text = "R21 RECOMMENDS"
            txtSubject.Text = "Enter Text..."
            Editor.Content = ""
            imgIcon.ImageUrl = "http://r21concierge.s3.amazonaws.com/images/concierge.png"
        End If
    End Sub
End Class
